...
for (let movie of movies) {
  console.log(`Movie ${++count}`, "After 2017", movie.year > 2017);
}
...
for(let movie of movies) {
  console.log(`Number of Actors in ${movie.title}`, movie.actors.length);
}

// 26. Log the number of genres in all three movies one by one
for(let movie of movies) {
  console.log(`Number of Genres in ${movie.title}`, movie.genres.length);
}
